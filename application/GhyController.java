...
public class GhyController {
	...
	@FXML
	private ChoiceBox<String> jinji=new ChoiceBox<String>();
	
	@FXML
	private Button but_;
	@FXML
	private Button GuaHaoButton;
	@FXML
	...
	public void initialize() {
		...
		choicebox4.getItems().addAll("内科","消化内科","呼吸内科","肝病","胃病");
		jinji.getItems().addAll("非紧急");
		jinji.getSelectionModel().selectFirst();
	...
	public void guahao() {
		...
		if(!age()) {
			guahaosuccess.setText("挂号失败，请输入正确年龄！");
		}
		else {
		if(Patient.GuaHao(choiceboxdoc.getValue(), NameText.getText(), AgeText.getText(), RandomText.getText(),choicebox1.getValue(),jinji.getValue(),IdpText.getText())&&NameText.getText()!=null) {
			
			List<Patient>listp=Patient.read();
			
			for(Patient pp:listp) {
				if(pp.getName().equals(NameText.getText())) {
					hzbh.setText("患者编号是："+pp.getIdP());
					break;
				}
			}
			...
			guahaosuccess.setText("挂号成功！");
				...
				guahaosuccess.setText("挂号失败，请输入完整并且正确的信息！");
